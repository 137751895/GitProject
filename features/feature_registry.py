...
class FeatureRegistry:                                        # [新增]
    ...
    def compute_registered_features(                          # [新增]
        ...
        **kwargs
        ...
        frames = []                                           # [新增]
        # Accumulate computed features so dependencies within  # [新增] [BUGFIX]
        # the registry (e.g. wavelet_entropy → wavelet_energy) # [新增]
        # can be resolved incrementally.                       # [新增]
        running_df = features_df.copy()                       # [新增]
        for key, entry in self._entries.items():              # [新增]
            # 检查依赖是否满足                                # [新增]
            missing = [                                       # [新增]
                d for d in entry.depends_on                   # [新增]
                if d not in running_df.columns                # [新增] [BUGFIX]
            ]                                                 # [新增]
            if missing:                                       # [新增]
                logger.warning(                               # [新增]
            ...
            try:                                              # [新增]
                result = entry.func(                          # [新增]
                    df, features_df=running_df, **kwargs      # [新增] [BUGFIX]
                )                                             # [新增]
                if result is not None and len(result.columns) > 0:  # [新增]
                    ...
                    # Update running_df so later features can  # [新增] [BUGFIX]
                    # reference these newly computed columns.  # [新增]
                    for col in result.columns:                # [新增]
                        running_df[col] = result[col]         # [新增]
            except Exception:                                 # [新增]
