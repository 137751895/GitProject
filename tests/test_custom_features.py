...
# ============================================================  # [新增]
# 第六辑: 单品种期货20因子 + 2可选扩展                          # [新增]
# ============================================================  # [新增]

def _make_batch6_df(n=200):                                      # [新增]
    """创建第六辑测试数据"""                                      # [新增]
    np.random.seed(60)                                           # [新增]
    close = 100.0 + np.cumsum(np.random.randn(n) * 0.5)         # [新增]
    oi = 5000.0 + np.cumsum(np.random.randn(n) * 20)            # [新增]
    volume = np.abs(np.random.randn(n) * 500 + 1000)            # [新增]
    high = close + np.abs(np.random.randn(n))                    # [新增]
    low = close - np.abs(np.random.randn(n))                     # [新增]
    return pd.DataFrame({                                        # [新增]
        "open": close + np.random.randn(n) * 0.1,               # [新增]
        "high": high,                                            # [新增]
        "low": low,                                              # [新增]
        "close": close,                                          # [新增]
        "volume": volume,                                        # [新增]
...
class TestBatch6OIVelocity:                                      # [新增]
    """open_interest_velocity tests"""                            # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_open_interest_velocity  # [新增]
        df = _make_batch6_df(30)                                 # [新增]
        result = compute_open_interest_velocity(df)              # [新增]
        assert "open_interest_velocity" in result.columns        # [新增]
        assert len(result) == len(df)                            # [新增]
        assert result["open_interest_velocity"].isna().sum() >= 5  # [新增]

    def test_hand_calc(self):                                    # [新增]
        from features.custom_features import compute_open_interest_velocity  # [新增]
        df = pd.DataFrame({"open_interest": [100, 110, 120, 130, 140, 160],  # [新增]
                           "close": [1]*6, "volume": [1]*6,     # [新增]
                           "high": [1]*6, "low": [1]*6, "open": [1]*6})  # [新增]
        r = compute_open_interest_velocity(df, window=5)         # [新增]
        # at index 5: (160-100)/5 = 12.0                        # [新增]
        assert abs(r["open_interest_velocity"].iloc[5] - 12.0) < 1e-6  # [新增]


class TestBatch6OIAcceleration:                                  # [新增]
    """open_interest_acceleration tests"""                        # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_open_interest_acceleration  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_open_interest_acceleration(df)          # [新增]
        assert "open_interest_acceleration" in result.columns    # [新增]
        assert result["open_interest_acceleration"].notna().sum() > 0  # [新增]


class TestBatch6LongShortImbalance:                              # [新增]
    """long_short_imbalance tests"""                              # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_long_short_imbalance  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_long_short_imbalance(df)                # [新增]
        assert "long_short_imbalance" in result.columns          # [新增]
        vals = result["long_short_imbalance"].dropna()           # [新增]
        assert len(vals) > 0                                     # [新增]


class TestBatch6NewPositionRatio:                                # [新增]
    """new_position_ratio tests"""                                # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_new_position_ratio  # [新增]
        df = _make_batch6_df(30)                                 # [新增]
        result = compute_new_position_ratio(df)                  # [新增]
        assert "new_position_ratio" in result.columns            # [新增]
        vals = result["new_position_ratio"].dropna()             # [新增]
        assert (vals >= 0).all()                                 # [新增]


class TestBatch6LiquidationPressure:                             # [新增]
    """liquidation_pressure tests"""                              # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_liquidation_pressure  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_liquidation_pressure(df)                # [新增]
        assert "liquidation_pressure" in result.columns          # [新增]


class TestBatch6HedgingRatio:                                    # [新增]
    """hedging_ratio_proxy tests"""                               # [新增]
    def test_positive(self):                                     # [新增]
        from features.custom_features import compute_hedging_ratio_proxy  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_hedging_ratio_proxy(df)                 # [新增]
        assert "hedging_ratio_proxy" in result.columns           # [新增]
        vals = result["hedging_ratio_proxy"].dropna()            # [新增]
        assert (vals > 0).all()                                  # [新增]


class TestBatch6SpeculationIndex:                                # [新增]
    """speculation_index tests"""                                  # [新增]
    def test_positive(self):                                     # [新增]
        from features.custom_features import compute_speculation_index  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_speculation_index(df)                   # [新增]
        assert "speculation_index" in result.columns             # [新增]
        vals = result["speculation_index"].dropna()              # [新增]
        assert (vals > 0).all()                                  # [新增]


class TestBatch6VolumeOICorrelation:                             # [新增]
    """volume_oi_correlation tests"""                              # [新增]
    def test_range(self):                                        # [新增]
        from features.custom_features import compute_volume_oi_correlation  # [新增]
        df = _make_batch6_df(100)                                # [新增]
        result = compute_volume_oi_correlation(df)               # [新增]
        assert "volume_oi_correlation" in result.columns         # [新增]
        vals = result["volume_oi_correlation"].dropna()          # [新增]
        assert (vals >= -1.0001).all() and (vals <= 1.0001).all()  # [新增]


class TestBatch6OISeasonalPattern:                               # [新增]
    """oi_seasonal_pattern tests"""                                # [新增]
    def test_range(self):                                        # [新增]
        from features.custom_features import compute_oi_seasonal_pattern  # [新增]
        df = _make_batch6_df(200)                                # [新增]
        result = compute_oi_seasonal_pattern(df)                 # [新增]
        assert "oi_seasonal_pattern" in result.columns           # [新增]
        vals = result["oi_seasonal_pattern"].dropna()            # [新增]
        assert (vals >= 0).all() and (vals <= 1).all()           # [新增]


class TestBatch6RolloverActivity:                                # [新增]
    """rollover_activity tests"""                                  # [新增]
    def test_nonneg(self):                                       # [新增]
        from features.custom_features import compute_rollover_activity  # [新增]
        df = _make_batch6_df(100)                                # [新增]
        result = compute_rollover_activity(df)                   # [新增]
        assert "rollover_activity" in result.columns             # [新增]
        vals = result["rollover_activity"].dropna()              # [新增]
        assert (vals >= -0.001).all()                            # [新增]


class TestBatch6ContractRollingPressure:                         # [新增]
    """contract_rolling_pressure tests"""                          # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_contract_rolling_pressure  # [新增]
        df = _make_batch6_df(100)                                # [新增]
        result = compute_contract_rolling_pressure(df)           # [新增]
        assert "contract_rolling_pressure" in result.columns     # [新增]


class TestBatch6OIPriceRegime:                                   # [新增]
    """oi_price_regime tests"""                                    # [新增]
    def test_values(self):                                       # [新增]
        from features.custom_features import compute_oi_price_regime  # [新增]
        df = _make_batch6_df(30)                                 # [新增]
        result = compute_oi_price_regime(df)                     # [新增]
        assert "oi_price_regime" in result.columns               # [新增]
        vals = result["oi_price_regime"].dropna()                # [新增]
        assert set(vals.unique()).issubset({0.0, 1.0, 2.0, 3.0, 4.0})  # [新增]


class TestBatch6VolumeOIRatioZscore:                             # [新增]
    """volume_oi_ratio_zscore tests"""                              # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_volume_oi_ratio_zscore  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_volume_oi_ratio_zscore(df)              # [新增]
        assert "volume_oi_ratio_zscore" in result.columns        # [新增]
        vals = result["volume_oi_ratio_zscore"].dropna()         # [新增]
        assert len(vals) > 0                                     # [新增]


class TestBatch6OIPriceDivergenceStrength:                       # [新增]
    """oi_price_divergence_strength tests"""                        # [新增]
    def test_nonneg_or_nan(self):                                # [新增]
        from features.custom_features import compute_oi_price_divergence_strength  # [新增]
        df = _make_batch6_df(30)                                 # [新增]
        result = compute_oi_price_divergence_strength(df)        # [新增]
        assert "oi_price_divergence_strength" in result.columns  # [新增]
        vals = result["oi_price_divergence_strength"].dropna()   # [新增]
        assert (vals >= 0).all()                                 # [新增]


class TestBatch6OIExtremeRatio:                                  # [新增]
    """oi_extreme_ratio tests"""                                    # [新增]
    def test_range(self):                                        # [新增]
        from features.custom_features import compute_oi_extreme_ratio  # [新增]
        df = _make_batch6_df(200)                                # [新增]
        result = compute_oi_extreme_ratio(df)                    # [新增]
        assert "oi_extreme_ratio" in result.columns              # [新增]
        vals = result["oi_extreme_ratio"].dropna()               # [新增]
        assert (vals >= -0.001).all() and (vals <= 1.001).all()  # [新增]


class TestBatch6VolumeBreakout:                                  # [新增]
    """volume_breakout tests"""                                    # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_volume_breakout  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_volume_breakout(df)                     # [新增]
        assert "volume_breakout" in result.columns               # [新增]
        vals = result["volume_breakout"].dropna()                # [新增]
        assert len(vals) > 0                                     # [新增]


class TestBatch6VolumeStability:                                 # [新增]
    """volume_stability tests"""                                    # [新增]
    def test_positive(self):                                     # [新增]
        from features.custom_features import compute_volume_stability  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_volume_stability(df)                    # [新增]
        assert "volume_stability" in result.columns              # [新增]
        vals = result["volume_stability"].dropna()               # [新增]
        assert (vals > 0).all()                                  # [新增]


class TestBatch6PriceConviction:                                 # [新增]
    """price_conviction tests"""                                    # [新增]
    def test_range(self):                                        # [新增]
        from features.custom_features import compute_price_conviction  # [新增]
        df = _make_batch6_df(30)                                 # [新增]
        result = compute_price_conviction(df)                    # [新增]
        assert "price_conviction" in result.columns              # [新增]
        vals = result["price_conviction"].dropna()               # [新增]
        assert (vals >= 0).all() and (vals <= 1.001).all()       # [新增]

    def test_flat_candle(self):                                  # [新增]
        """high==low should produce NaN"""                        # [新增]
        from features.custom_features import compute_price_conviction  # [新增]
        df = pd.DataFrame({"open": [100], "high": [100],         # [新增]
                           "low": [100], "close": [100],         # [新增]
                           "volume": [1], "open_interest": [1]}) # [新增]
        result = compute_price_conviction(df)                    # [新增]
        assert result["price_conviction"].isna().all()           # [新增]


class TestBatch6OIMomentumRatio:                                 # [新增]
    """oi_momentum_ratio tests"""                                   # [新增]
    def test_positive(self):                                     # [新增]
        from features.custom_features import compute_oi_momentum_ratio  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_oi_momentum_ratio(df)                   # [新增]
        assert "oi_momentum_ratio" in result.columns             # [新增]
        vals = result["oi_momentum_ratio"].dropna()              # [新增]
        assert (vals > 0).all()                                  # [新增]

    def test_swap_fast_slow(self):                               # [新增]
        """fast > slow should auto-swap"""                        # [新增]
        from features.custom_features import compute_oi_momentum_ratio  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        r1 = compute_oi_momentum_ratio(df, fast=5, slow=20)      # [新增]
        r2 = compute_oi_momentum_ratio(df, fast=20, slow=5)      # [新增]
        pd.testing.assert_frame_equal(r1, r2)                    # [新增]


class TestBatch6VolumePriceEfficiency:                           # [新增]
    """volume_price_efficiency tests"""                              # [新增]
    def test_nonneg(self):                                       # [新增]
        from features.custom_features import compute_volume_price_efficiency  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_volume_price_efficiency(df)             # [新增]
        assert "volume_price_efficiency" in result.columns       # [新增]
        vals = result["volume_price_efficiency"].dropna()        # [新增]
        assert (vals >= 0).all()                                 # [新增]


class TestBatch6OIMeanReversion:                                 # [新增]
    """oi_mean_reversion tests (optional extension)"""             # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_oi_mean_reversion  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_oi_mean_reversion(df)                   # [新增]
        assert "oi_mean_reversion" in result.columns             # [新增]
        vals = result["oi_mean_reversion"].dropna()              # [新增]
        assert len(vals) > 0                                     # [新增]


class TestBatch6VolumePriceDivergence:                           # [新增]
    """volume_price_divergence tests (optional extension)"""       # [新增]
    def test_basic(self):                                        # [新增]
        from features.custom_features import compute_volume_price_divergence  # [新增]
        df = _make_batch6_df(50)                                 # [新增]
        result = compute_volume_price_divergence(df)             # [新增]
        assert "volume_price_divergence" in result.columns       # [新增]


class TestBatch6Integration:                                     # [新增]
    """Integration tests for all batch-6 features"""              # [新增]
    BATCH6_FEATURES = [                                          # [新增]
        "open_interest_velocity", "open_interest_acceleration",  # [新增]
        "long_short_imbalance", "new_position_ratio",            # [新增]
        "liquidation_pressure", "hedging_ratio_proxy",           # [新增]
        "speculation_index", "volume_oi_correlation",            # [新增]
        "oi_seasonal_pattern", "rollover_activity",              # [新增]
        "contract_rolling_pressure", "oi_price_regime",          # [新增]
        "volume_oi_ratio_zscore", "oi_price_divergence_strength",  # [新增]
        "oi_extreme_ratio", "volume_breakout",                   # [新增]
        "volume_stability", "price_conviction",                  # [新增]
        "oi_momentum_ratio", "volume_price_efficiency",          # [新增]
        "oi_mean_reversion", "volume_price_divergence",          # [新增]
    ]                                                            # [新增]

    def test_all_registered(self):                               # [新增]
        from features.feature_registry import FeatureRegistry    # [新增]
        import features.custom_features                          # [新增]
        registry = FeatureRegistry()                             # [新增]
        names = set()                                            # [新增]
        for _, meta in registry._entries.items():                # [新增]
            names.update(meta.output_names)                      # [新增]
        for feat in self.BATCH6_FEATURES:                        # [新增]
            assert feat in names, f"Not registered: {feat}"      # [新增]

    def test_compute_all_features_includes_batch6(self):         # [新增]
        from features.feature_engineering import compute_all_features  # [新增]
        df = _make_batch6_df(300)                                # [新增]
        features_df = compute_all_features(df, period="5min")    # [新增]
        for feat in self.BATCH6_FEATURES:                        # [新增]
            assert feat in features_df.columns, f"Missing: {feat}"  # [新增]
        assert len(features_df.columns) == len(set(features_df.columns))  # [新增]
